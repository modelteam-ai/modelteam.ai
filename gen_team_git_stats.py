...
def usage():
    print("Usage: gen_team_git_stats.py [-r <repo_list.txt>] -t <team_name> [-g <git_email_ids_as_csv>] [-n <num_years>]")
    ...
    print("If git_email_ids are not provided, profiles will be generated for all users in the repos")
...
def main():
    ...
    parser.add_argument("-g", "--git_email_ids", required=False, help="Git IDs of the users present in git log as csv")
    ...
    git_email_ids = args.git_email_ids
    num_years = args.num_years

    validate_input(num_years, repo_list)

    output_path = run_model_team_git_parser(repo_list, git_email_ids, num_years, args.dev, team_name)
